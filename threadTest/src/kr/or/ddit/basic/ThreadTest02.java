...
public class ThreadTest02 {
	public static void main(String[] args) {
		...
		//		 생성한다. 이 인스턴스의 start()메서드를 호출해서 실행한다.
		...
		//     인스턴스를 생성한다.
		//     Thread클래스의 인스턴스를 생성할 때 생성자의 인자값으로 넣어 생성한다.
...
class MyThread1 extends Thread{
	...
	@Override
	public void run() {
		// 이 run()메서드에는 이 스레드가 처리할 내용을 작성하면 된다.
		// 예시) '*'문자 200개 출력
		for(int i=1; i<=200; i++) {
			System.out.print("*");
			try {
				//Thread.sleep(시간); ==> 주어진 '시간'동안 작업을 잠시 멈춘다. //스레드에서 제공해주는 메소드
