...
//웹브라우저의 앞으로 가기, 뒤로 가기 기능 구현 클래스 작성 (스택이용)
class Browser {
	private Stack<String> back; // 이전 방문 내역이 저장될 스텍
	private Stack<String> forward; // 다음 방문 내역이 저장될 스텍
	...
	public void goURL(String url) {
		...
		if (currentURL != null && !"".equals(currentURL)) {
			back.push(currentURL); // 이전 사이트를 변수 back에 저장
		}
		currentURL = url; // 현재 페이지를 이동할 페이지로 변경한다.
		forward.clear();
	}

	// 뒤로 가기 ==> 현재 페이지를 forward 스택에 추가하고
	// back스택에서 주소를 꺼내와 현재 페이지로 변경한다.
	public void goBack() {
		if(!back.isEmpty()) { //back이 비어있지 않다면
...
//				   forward 스택에서 주소를 꺼내와 현재 페이지로 변경한다.
